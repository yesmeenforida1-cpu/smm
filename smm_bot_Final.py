...
peak_balance_usd = 0.0   # ✅ Feature 1: সর্বোচ্চ ব্যালেন্স ট্র্যাক
balance_alert_sent = False  # ✅ Feature 1: একবার অ্যালার্ট পাঠালে বারবার না পাঠাতে
...
async def check_balance_alert(context):
    """প্রতি ৩০ মিনিটে Peakerr ব্যালেন্স চেক করো — ৯০% খরচ হলে অ্যালার্ট"""
    global peak_balance_usd, balance_alert_sent
    try:
        usd, bdt = get_peakerr_balance()
        # সর্বোচ্চ ব্যালেন্স আপডেট করো
        if usd > peak_balance_usd:
            peak_balance_usd = usd
            balance_alert_sent = False  # রিচার্জ হয়েছে, নতুন অ্যালার্ট পাঠাবে
        # ৯০% খরচ হলে = মাত্র ১০% বাকি
        if peak_balance_usd > 0 and not balance_alert_sent:
            spent_pct = ((peak_balance_usd - usd) / peak_balance_usd) * 100
            if spent_pct >= 90:
                balance_alert_sent = True  # একবারই পাঠাও
                await context.bot.send_message(ADMIN_ID,
                    f"⚠️ LOW BALANCE ALERT! ⚠️\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"💎 Peakerr ব্যালেন্স ৯০% শেষ!\n"
                    f"📊 সর্বোচ্চ ছিল: ${peak_balance_usd:.2f}\n"
                    f"💵 এখন বাকি: ${usd:.2f} = {bdt:.2f} TK\n"
                    f"📉 খরচ হয়েছে: {spent_pct:.1f}%\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━\n"
