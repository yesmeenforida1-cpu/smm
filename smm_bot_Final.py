...
def auto_protect():
    updated = []
    try:
        get_smm_services()  # smmhid থেকে fresh দাম আনো
        for pk, pv in SERVICES.items():
            for sk, sv in pv["list"].items():
                ...
                if rate:
                    ...
                    old = sv["price"]
                    SERVICES[pk]["list"][sk]["price"] = new_price
                    if abs(new_price - old) > 0.5:  # দাম বদলালে দেখাও
                        updated.append(f"{'📈' if new_price > old else '📉'} {sv['name']}: {old} → {new_price} TK")
    except:
        pass
    return updated
...
def get_price(skey):
    # smmhid থেকে real-time দাম আনো + ৩৫% profit
    for pk, pv in SERVICES.items():
        if skey in pv["list"]:
            sv = pv["list"][skey]
            try:
                ...
                if rate:
                    smm_bdt = round(rate * 110, 2)
                    final_price = round(smm_bdt * (1 + PROFIT_PCT / 100), 2)
                    SERVICES[pk]["list"][skey]["price"] = final_price  # cache
                    return final_price
            except:
                pass
            return sv["price"]  # fallback
    return 0
...
def main():
    # বট চালু হলেই smmhid থেকে real-time দাম লোড করো
    print("⏳ smmhid থেকে দাম লোড হচ্ছে...")
    auto_protect()
    print("✅ দাম লোড সম্পন্ন!")
